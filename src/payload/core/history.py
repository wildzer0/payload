...
class HistoryStore:
    ...
    def is_dirty(self, table_name: str, source_path: Path, output_paths: list[Path] | None = None) -> bool:
        """True se il sorgente attuale differisce dall'ultimo snapshot,
        se non esiste ancora nessuno snapshot per questa tabella, o se
        uno degli output attuali (output_paths, se dato) differisce da
        quello che l'ultimo snapshot aveva registrato — es. cambiare
        writer senza toccare il sorgente produce un output diverso
        (nome ed estensione compresi) che altrimenti passerebbe
        inosservato: il sorgente è identico, quindi solo il confronto
        source_blob non basta a segnalare che c'è qualcosa di nuovo da
        salvare."""
        last = self.last_snapshot(table_name)
        if last is None:
            return True
        current_hash = _hash_bytes(source_path.read_bytes())
        if current_hash != last.source_blob:
            return True
    ...
    def restore(
        ...
        """Riporta sorgente e output generati allo stato dello snapshot e
