...
# fails at the module's own IMPORT (not while instantiating a valid
# class) — different from BROKEN_SOURCE, which instead exercises _register_one
MODULE_LEVEL_FAILURE_SOURCE = '''
raise RuntimeError("failed before even defining a plugin")
'''
...
def test_env_var_nonexistent_directory_ignored(tmp_path, monkeypatch):
    monkeypatch.setenv("PAYLOAD_PLUGIN_PATH", str(tmp_path / "does_not_exist"))
...
def test_single_writer_loaded_and_functional(tmp_path):
    ...
    from payload.core.ir import TableIR
    ir = TableIR(name="t", data=b"hello", source_path=Path("x"), source_format="fake")
...
def test_module_import_failure_non_strict_tracked_not_raised(tmp_path):
    ...
    registry = PluginRegistry()
    load_local_plugins(tmp_path, registry, strict=False)  # must not raise
...
def test_underscore_prefixed_files_ignored(tmp_path):
    ...
    registry = PluginRegistry()
    load_local_plugins(tmp_path, registry, strict=True)  # must not raise
...
# --- find_stub_methods (unimplemented scaffold) -----------------------------
...
def test_find_stub_methods_ignores_unrelated_method_names(tmp_path):
    """Only parse/emit/run count — any helper method that raises
    NotImplementedError isn't an unimplemented plugin."""
...
def test_find_stub_methods_detects_bare_raise_without_call(tmp_path):
    """'raise NotImplementedError' (no parentheses/message) is
    equivalent for detection purposes — not just the Call form
...
def test_find_stub_methods_ignores_multi_statement_body(tmp_path):
    """A method with real logic before an eventual raise isn't a
    stub, even if it still ends up raising something."""
    path = tmp_path / "plugin.py"
    path.write_text(
        "class X:\n"
        "    def run(self, config):\n"
...
def test_find_stub_methods_skips_leading_docstring_before_raise(tmp_path):
    """The scaffold has no docstring inside the method, but a user
    starting from scratch might add one before the raise — that must
    not prevent detecting the stub."""
    path = tmp_path / "plugin.py"
    path.write_text(
        "class X:\n"
        "    def run(self, config):\n"
        "        '''TODO: describe what this check does.'''\n"
...
def test_find_stub_methods_ignores_raise_of_non_name_expression(tmp_path):
    """A raise whose expression is neither a Name nor a Call(Name)
    (e.g. an attribute like 'module.SomeError(...)') isn't recognized
    as NotImplementedError — conservative behavior, no false
    positives."""
    path = tmp_path / "plugin.py"
    path.write_text(
        "import errors\n"
